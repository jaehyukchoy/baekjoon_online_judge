...
def make_table(start,end):
    ...
    while start<end:
        if s[start:start+4]=='<tr>':
            ...
            start+=5
            # while type(tr)==type([]) and len(tr)==1:
            #     tr=tr[0]
            table.append(tr)
            ...
            if s[start:start+7]=='<table>':            
                ...
                while type(tmp)==type([]) and len(tmp)==1:
                    tmp=tmp[0]
                tr.append(tmp)
                # tr.append(make_table(start,start+table_end+8))
                start+=table_end+13
            else:
...
def make_code(table):    
    while type(table)==type([]) and len(table)==1:
        table=table[0]
    print(table)
    code=''
    if type(table)==type([]):
        if sum(map(lambda x:0 if type(x)==type([]) else 1,table)):
...
print('<body>')
for s in ss:
    #print(make_table(7,len(s)))
    print('<table>'+make_code(make_table(7,len(s)))+'</table>')
print('</body>')